...
class AnnouncementWatcher:
    ...
    def __SetScheduling(releaseInfo):
        ...
        # Cooperation.
        if (
            releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberCount > 1
            and releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberId >= 0
            and releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberId
            < releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberCount
            ...
            releaseName = (
                releaseInfo.ReleaseName.replace(".", "")
                .replace("-", "")
                .replace(" ", "")
                .lower()
            )
            ...
            if releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberId >= id:
                startDelay = (
                    releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberId - id
                ) * releaseInfo.AnnouncementSource.AutomaticJobCooperationDelay
            else:
                startDelay = (
                    releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberCount
                    - id
                    + releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberId
                ) * releaseInfo.AnnouncementSource.AutomaticJobCooperationDelay

        if startDelay > 0:
            releaseInfo.JobRunningState = JobRunningState.Scheduled
            releaseInfo.ScheduleTimeUtc = (
                datetime.datetime.utcnow() + datetime.timedelta(seconds=startDelay)
            )
    ...
    def __HandleFileSource(releaseInfo, announcementFilePath):
        if AnnouncementWatcher.__HandleFileSourceInternal(
            releaseInfo, announcementFilePath
        ):
            return True
    ...
    def __ProcessAnnouncementFileInternal(announcementFilePath, announcementFilename):
        matches = re.match(
            r"\[source=(.+?)\]\[id=(.+?)\]\[title=(.+)\]", announcementFilename
        )
        if not matches:
            MyGlobals.Logger.info(
                "Invalid announcement name format: '%s'." % announcementFilename
            )
            return None
        ...
        if announcementSource is None:
            MyGlobals.Logger.error(
                "Unknown announcement source: '%s'." % announcementSourceName
            )
            return None
        ...
        if announcementSourceName == "file":
            if not AnnouncementWatcher.__HandleFileSource(
                releaseInfo, announcementFilePath
            ):
                MyGlobals.Logger.error(
                    "Invalid announcement file: '%s'." % announcementFilePath
                )
                return None
        elif announcementId == "0":
            # For announcements made by autodl-irssi the torrent ID is in the file.
            announcementId = AnnouncementWatcher.__TryGettingIdFromContents(
                announcementFilePath, announcementSource
            )
            if len(announcementId) <= 0:
                ...
                if len(announcementId) <= 0:
                    MyGlobals.Logger.error(
                        "Invalid torrent ID in announcement: '%s'."
    ...
    def ProcessAnnouncementFile(path):
        ...
        releaseInfo = AnnouncementWatcher.__ProcessAnnouncementFileInternal(
            path, filename
        )
        if releaseInfo is None:
            invalidFilePath = os.path.join(
                Settings.GetAnnouncementInvalidPath(), filename
            )
    ...
    def LoadAnnouncementFilesIntoTheDatabase():
        ...
        for item in files:
            path = item[
                1
            ]  # First element is the modification time, second is the path.
