...
class MoviePoster:
    ...
    def Get(logger, imdbId):
        logger.info(
            "Getting movie poster from The Internet Movie Poster DataBase for IMDb id '%s'."
            % imdbId
        )
        ...
        try:
            ...
            logger.exception(
                "Got exception while trying to get poster from The Internet Movie Poster DataBase by IMDb id '%s'."
                % imdbId
            )
