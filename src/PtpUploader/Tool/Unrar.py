...
class Unrar:
    ...
    def Extract(rarPath, destinationPath):
        ...
        if errorCode != 0:
            raise PtpUploaderException(
                "CProcess execution '%s' returned with error code '%s'."
                % (args, errorCode)
            )
        ...
        for file in files:
            ...
            if os.path.exists(destinationFilePath):
                raise PtpUploaderException(
                    "Can't move file '%s' to '%s' because destination already exists."
    ...
    def IsFirstRar(path):
        ...
        if fnmatch.fnmatch(path, "*.rar"):
            if fnmatch.fnmatch(path, "*.part01.rar") or fnmatch.fnmatch(
                path, "*.part001.rar"
            ):
                return True
