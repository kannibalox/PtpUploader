...
class Transmission:
	...
	def AddTorrent(self, logger=None, torrentPath=None, downloadPath=None):
		logger.info( "Initiating the download of torrent '%s' with Transmission to '%s'." % ( torrentPath, downloadPath ) );
	...
	def AddTorrentSkipHashCheck(self, logger, torrentPath, downloadPath):
		logger.info( "Adding torrent '%s' without hash checking (not really) to Transmission to '%s'." % ( torrentPath, downloadPath ) );
