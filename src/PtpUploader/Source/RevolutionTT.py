...
class RevolutionTT(SourceBase):
	def __init__(self):
		SourceBase.__init__( self )

		self.Name = "rtt"
		self.NameInSettings = "RevolutionTT"

	def IsEnabled(self):
		return len( self.Username ) > 0 and len( self.Password ) > 0

	def Login(self):
		MyGlobals.Logger.info( "Logging in to RTT." )

		# RTT stores a cookie when login.php is loaded that is needed for takelogin.php.
		MyGlobals.session.get( "https://revolutiontt.me/login.php" )

		postData = { "username": self.Username, "password": self.Password }
		result = MyGlobals.session.post( "https://revolutiontt.me/takelogin.php", data=postData )
	...
	def CheckIfLoggedInFromResponse(self, response):
		if response.find( """action='takelogin.php'""" ) != -1 or response.find( """<a href='login.php'>Back to Login</a>""" ) != -1:
			raise PtpUploaderException( "Looks like you are not logged in to RTT. Probably due to the bad user name or password in settings." )

	def __GetTorrentPageAsString( self, logger, releaseInfo ):
		url = "https://revolutiontt.me/details.php?id=%s" % releaseInfo.AnnouncementId
	...
	def __TryGettingImdbIdFromNfoPage( self, logger, releaseInfo ):
		url = "https://revolutiontt.me/viewnfo.php?id=%s" % releaseInfo.AnnouncementId
	...
	def __ReadTorrentPageInternal( self, logger, releaseInfo, description ):
		# Get release name.
		matches = re.search( r"<title>Revolution :: Details for torrent &quot;(.+)&quot;</title>", description )
		if matches is None:
		...
		if matches is None:
			logger.warning( "Size not found on torrent page '%s'." % matches )
		else:
			size = matches.group( 1 )
			logger.info( "Downloading torrent size '%s' from RTT." % size)
		...
		matches = re.search( r"""<tr><td class=rowhead width=1%>Download</td><td width=99% align=left><a class="index" href="https://revolutiontt.me/download.php/(.+?)">""", description )
		if matches is None:
			raise PtpUploaderException( JobRunningState.Ignored_MissingInfo, "Download link can't be found on torrent page." )

		releaseInfo.SceneAccessDownloadUrl = "https://revolutiontt.me/download.php/" + matches.group( 1 )
	...
	def __ReadTorrentPage( self, logger, releaseInfo ):
		...
		# For some reason there are announced, but non visible releases on RTT that never start seeding.
	...
	def __HandleAutoCreatedJob(self, logger, releaseInfo):
		...
		if releaseName != releaseInfo.ReleaseName:
			raise PtpUploaderException( "Announcement release name '%s' and release name '%s' on RTT are different." % ( releaseInfo.ReleaseName, releaseName ) )
	...
	def DownloadTorrent(self, logger, releaseInfo, path):
		...
		logger.info( "Downloading torrent file '%s' from RTT." % releaseInfo.SceneAccessDownloadUrl)

		url = releaseInfo.SceneAccessDownloadUrl
		result = MyGlobals.session.get( url )
		result.raise_for_status()
	...
	def GetIdFromUrl(self, url):
		result = re.match( r".*revolutiontt\.me/details\.php\?id=([0-9a-zA-Z]+).*", url )
		if result is None:
			return ""
		else:
	...
	def GetUrlFromId(self, id):
		return "https://revolutiontt.me/details.php?id=" + id

	def GetIdFromAutodlIrssiUrl( self, url ):
		# https://revolutiontt.me/download.php/543778/The.Theory.of.Everything.2014.1080p.BluRay.X264-AMIABLE.torrent?passkey=xxxxxxxxxxxxx
		result = re.match( r".*revolutiontt\.me/download\.php/([0-9a-zA-Z]+)/.*", url )
		if result is None:
			return ""
		else:
			return result.group( 1 )