...
class DigitalHive(SourceBase):
	...
	def Login(self):
		...
		result = MyGlobals.session.post( "https://www.digitalhive.org/takelogin.php", data=postData )
	...
	def __CheckIfLoggedInFromResponse(self, response):
		if response.find( """action='takelogin.php'""" ) != -1 or response.find( """<a href='login.php'>Back to Login</a>""" ) != -1:
			raise PtpUploaderException( "Looks like you are not logged in to DigitalHive. Probably due to the bad user name or password in settings." )

	def __GetTorrentPageAsString( self, logger, releaseInfo ):
		url = "https://www.digitalhive.org/details.php?id=%s" % releaseInfo.AnnouncementId
	...
	def __TryGettingImdbIdFromNfoPage( self, logger, releaseInfo ):
		url = "https://www.digitalhive.org/viewnfo.php?id=%s" % releaseInfo.AnnouncementId
	...
	def __HandleAutoCreatedJob(self, logger, releaseInfo):
		...
		if releaseName != releaseInfo.ReleaseName:
			raise PtpUploaderException( "Announcement release name '%s' and release name '%s' on DigitalHive are different." % ( releaseInfo.ReleaseName, releaseName ) )
	...
	def DownloadTorrent(self, logger, releaseInfo, path):
		url = "https://www.digitalhive.org/download.php?id=%s&https=no" % releaseInfo.AnnouncementId
