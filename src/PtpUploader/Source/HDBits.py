    ...
    def __CheckIfLoggedInFromResponse(self, response):
        if response.json()["status"] != 0:
            raise PtpUploaderException(
                "Looks like you are not logged in to HDBits. Probably due to the bad user name or password in settings."
            )
