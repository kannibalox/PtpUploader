...
class AlphaRatio(SourceBase):
    ...
    def Login(self):
        ...
        result = MakeRetryingHttpPostRequestWithRequests(
            "https://alpharatio.cc/login.php", postData
        )
    ...
    def __CheckIfLoggedInFromResponse(self, response):
        if response.find('action="login.php"') >= 0:
            raise PtpUploaderException(
                "Looks like you are not logged in to AlphaRatio. Probably due to the bad user name or password in settings."
            )
    ...
    def __ReadTorrentPage(self, logger, releaseInfo):
        url = (
            "https://alpharatio.cc/torrents.php?torrentid=%s"
    ...
    def __HandleAutoCreatedJob(self, logger, releaseInfo):
        ...
        if releaseName != releaseInfo.ReleaseName:
            raise PtpUploaderException(
                "Announcement release name '%s' and release name '%s' on AlphaRatio are different."
                % (releaseInfo.ReleaseName, releaseName)
            )
    ...
    def DownloadTorrent(self, logger, releaseInfo, path):
        ...
        url = (
            "https://alpharatio.cc/torrents.php?action=download&id="
            + releaseInfo.AnnouncementId
        )
        result = MakeRetryingHttpGetRequestWithRequests(url)
    ...
    def GetIdFromAutodlIrssiUrl(self, url):
        ...
        result = re.match(
            r".*alpharatio\.cc\/torrents\.php\?action=download\&id=(\d+).*", url
        )
        if result is None:
            return ""
        else:
