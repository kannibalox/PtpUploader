...
class FunFile(SourceBase):
    ...
    def Login(self):
        ...
        result = MyGlobals.session.post(
            "https://www.funfile.org/takelogin.php", data=postData
        )
    ...
    def __CheckIfLoggedInFromResponse(self, response):
        if response.find('action="takelogin.php"') != -1:
            raise PtpUploaderException(
                "Looks like you are not logged in to FunFile. Probably due to the bad user name or password in settings."
            )
    ...
    def __ReadTorrentPage(self, logger, releaseInfo):
        url = (
            "https://www.funfile.org/details.php?id=%s&filelist=1"
        ...
        # Get release name.
        matches = re.search(
            r"""Details for torrent &quot;(.+)&quot;</title>""", description
        )
        if matches is None:
            raise PtpUploaderException(
                JobRunningState.Ignored_MissingInfo,
        ...
        matches = re.search(
            r"""<tr><td class="rowhead">Action</td><td class="row1"><span style="float:left"><a class="index" href="download.php/(.+?)">""",
            description,
        )
        if matches is None:
            raise PtpUploaderException(
        ...
        releaseInfo.SceneAccessDownloadUrl = (
            "https://www.funfile.org/download.php/" + matches.group(1)
        )
    ...
    def DownloadTorrent(self, logger, releaseInfo, path):
        ...
        result = MakeRetryingHttpGetRequestWithRequests(
            releaseInfo.SceneAccessDownloadUrl
        )
